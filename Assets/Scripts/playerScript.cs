...
public class playerScript : MonoBehaviour
{
   ...
   public float xInput, yInput, scale, timer;
    ...
    [SerializeField] fireworkScript fireworkScript;
        ...
        if (collision.gameObject.CompareTag("Gem"))
        {
            if (collisionManager.queue.Count > 0)
            {
                    ...
                    collisionManager.objectsInSequence[collisionManager.currentIndex].GetComponent<techieScript>().color)
                {
                    ...
                    if (comboScript._currentCombo >= 10) fireworkScript.TryTriggerFirework(new Vector3(Random.Range(-2, 2), transform.position.y + 3), comboScript._currentCombo);
                    if (!hasStarted)
                    {
